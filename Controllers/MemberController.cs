...
namespace testCitybreak.Controllers
{
	public class MemberController : Controller
	{
		...
		public MemberController(CitybreakContext context, ILogger<MemberController> logger,
			PasswordHasher<memberTable> passwordHasher)
		{
			_context = context;
		...
		public async Task<IActionResult> Register([FromBody] memberTable value)
		{
			...
			try
			{
				...
				{
					memberData.password = _passwordHasher.HashPassword(value, value.password);
				}
				else
				{
		...
		{
			string? redirectUrl = Url.Action("GoogleResponse", "Member");
		...
		public async Task<IActionResult> resetPassword([FromBody] memberTable value)
		{
			...
			try
			{
				...
				user.password = _passwordHasher.HashPassword(value, value.password);
				await _context.SaveChangesAsync();
				return Ok(new
				{
					success = true,
